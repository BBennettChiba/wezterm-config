...
local toRgba = function(hex, opacity)
	local r, g, b = hex2rgb(hex)
	return "rgba(" .. r .. "," .. g .. "," .. b .. "," .. tostring(opacity) .. ")"
end
...
local ui = {
	apply_to_config = function(config)
		...
		config.colors = {
			...
			tab_bar = {
				background = toRgba(scheme.background, 0.8),
			},
		}
		...
		-- doesn't work in current build
		-- config.background = {
		-- 	{
		-- 		source = {
		-- 			File = wezterm.home_dir .. "/.config/wezterm/backgrounds/japan.gif",
		-- 		},
		-- 	},
		-- }
	end,
...
wezterm.on("format-tab-title", function(tab, tabs, panes, config, hover, max_width)
	...
	table.insert(tabTable, {
		Text = " "
			.. (tab.is_active and title .. " " or icon_or_nothing(title))
			.. tostring((tab.tab_index + 1))
			.. " ",
	})
	if is_last then
		table.insert(tabTable, { Background = { Color = toRgba(scheme.background, 0.8) } })
